...
reimbursementRouter.get('/status/:statusId*', async(req : Request, res : Response) => {
    ...
    if(isNaN(statusId)){
        ...
        try{
            let query : string = `SELECT reimbursementid, users.username as "author", amount, datesubmitted, dateresolved, description, resolvertable.username as "resolver", reimbursementstatus.status, reimbursementtype.type FROM (((reimbursement LEFT JOIN reimbursementstatus ON reimbursement.status = statusid) LEFT JOIN reimbursementtype ON reimbursement.type = typeid) LEFT JOIN users ON reimbursement.author = users.userid) LEFT JOIN users as resolvertable ON reimbursement.resolver = resolvertable.userid WHERE status = ${statusId}`
            if(typeof(req.query.start) != 'undefined'){
...
reimbursementRouter.get('/author/userId/:userId*', async (req : Request, res : Response) => {
    ...
    if(isNaN(userId)){
        ...
        try{
            let query : string = `SELECT reimbursementid, users.username as "author", amount, datesubmitted, dateresolved, description, resolvertable.username as "resolver", reimbursementstatus.status, reimbursementtype.type FROM (((reimbursement LEFT JOIN reimbursementstatus ON reimbursement.status = statusid) LEFT JOIN reimbursementtype ON reimbursement.type = typeid) LEFT JOIN users ON reimbursement.author = users.userid) LEFT JOIN users as resolvertable ON reimbursement.resolver = resolvertable.userid WHERE author = ${userId}`
            if(typeof(req.query.start) != 'undefined'){
                query += ` and dateSubmitted = ${startDate}`;
            }
            if(typeof(req.query.end) != 'undefined'){
